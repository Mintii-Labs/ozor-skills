...
**Good:** "Create a 45-second landscape (16:9) video with 4 scenes"
...
(2) The Problem — Split screen: left side shows a user manually sorting through emails (cluttered inbox visual), right side shows a clock counting up. Text: "Hours lost to repetitive tasks."

(3) Smart Assist in Action — Show a clean UI with an AI suggestion popup appearing. The popup reads: "I noticed you send this report every Monday. Want me to automate it?" A cursor clicks "Yes". Smooth animation.

(4) CTA — Large centered text: "Try Smart Assist today." Subtext: "product.com/smart-assist". Logo centered below. Subtle glow animation on the URL.
...
(2) How It Works — Show a split-panel code editor. Left: component library with pre-built UI components. Right: live preview updating in real-time. Text overlay: "Pre-built components. Real-time preview. Zero config deployment."

(3) Key Features — 3 features appear as animated cards: "Real-time Collaboration" (icon: cursor pair), "Built-in Testing" (icon: checkmark), "One-Click Deploy" (icon: rocket). Each card slides in sequentially.

(4) Social Proof — "50,000+ developers. Featured in TechCrunch." Developer count animates up with a counter effect. TechCrunch logo appears beside the count.

(5) CTA — "Start building for free." URL: "[platform.com]". Centered, clean, logo above.
...
5. **Don't over-engineer.** A clear, specific 4-scene prompt beats a complex 8-scene prompt. Simpler is better.
